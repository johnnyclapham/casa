    ...
    private final void onLoadChildren(final IMediaBrowserServiceCompatCallbacks paramIMediaBrowserServiceCompatCallbacks, final String paramString, final List paramList)
    {
      this.mHandler.post(new Runnable()
      {
        public void run()
        {
          ...
          List localList;
          MediaBrowserCompat.MediaBrowserImplBase.Subscription localSubscription;
          do
          {
            return;
            localList = paramList;
            ...
            localSubscription = (MediaBrowserCompat.MediaBrowserImplBase.Subscription)MediaBrowserCompat.MediaBrowserImplBase.this.mSubscriptions.get(paramString);
          } while (localSubscription == null);
          localSubscription.callback.onChildrenLoaded(paramString, localList);
        }
      });
    }
    ...
    public void getItem(@NonNull final String paramString, @NonNull final MediaBrowserCompat.ItemCallback paramItemCallback)
    {
      ...
      ResultReceiver local3 = new ResultReceiver(this.mHandler)
      {
        protected void onReceiveResult(int paramAnonymousInt, Bundle paramAnonymousBundle)
        {
          ...
          Parcelable localParcelable = paramAnonymousBundle.getParcelable("media_item");
          if (!(localParcelable instanceof MediaBrowserCompat.MediaItem))
          {
            paramItemCallback.onError(paramString);
            return;
          }
          paramItemCallback.onItemLoaded((MediaBrowserCompat.MediaItem)localParcelable);
        }
      };
      try
      {
